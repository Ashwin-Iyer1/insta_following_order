(()=>{(async()=>{let p=e=>new Promise(o=>setTimeout(o,e)),i=location.pathname.split("/").filter(Boolean),u=["explore","reels","direct","stories","accounts","tv","p","ar","about","hashtag","locations","audio"],f=i.length>=1&&!u.includes(i[0]),l,a;if(f){let e=i[0];console.log(`On profile page \u2014 looking up user ID for @${e} ...`);let n=(await(await fetch(`https://www.instagram.com/web/search/topsearch/?query=${encodeURIComponent(e)}`,{credentials:"include"})).json()).users?.find(s=>s.user.username===e)?.user;if(!n)throw new Error(`User "${e}" not found in search results`);l=n.pk,a=e,console.log(`User ID: ${l}`)}else{let e=("; "+document.cookie).split("; ds_user_id=").length===2?("; "+document.cookie).split("; ds_user_id=").pop().split(";").shift():null;if(!e)throw new Error("Not logged in \u2014 ds_user_id cookie not found");l=e,a=e,console.log(`User ID: ${l}`)}let g="d04b0a864b4b54837c0d870b0e77e076",t=[],m=!0,c=null;for(;m;){let e={id:l,include_reel:!0,fetch_mutual:!0,first:50};c&&(e.after=c);let o=`https://www.instagram.com/graphql/query/?query_hash=${g}&variables=${encodeURIComponent(JSON.stringify(e))}`;await p(2e3+Math.random()*3e3);let r=await fetch(o,{credentials:"include"});if(!r.ok){console.error(`Request failed: ${r.status}`);break}let n=(await r.json()).data.user.edge_follow;for(let s of n.edges)t.push({username:s.node.username,full_name:s.node.full_name||""});m=n.page_info.has_next_page,c=n.page_info.end_cursor,console.log(`  Fetched ${t.length} so far ...`)}console.log(`
Total following: ${t.length}
`),console.log(`Most recently followed \u2192 oldest:
`),console.table(t.map((e,o)=>({"#":o+1,username:`@${e.username}`,full_name:e.full_name})));let h=new Blob([JSON.stringify(t,null,2)],{type:"application/json"}),d=document.createElement("a");d.href=URL.createObjectURL(h),d.download=`${a}_following.json`,d.click(),console.log(`
\u2713 Downloaded ${a}_following.json`);let b=t.map((e,o)=>`
            <tr data-index="${o+1}" data-username="${e.username.toLowerCase()}" data-fullname="${(e.full_name||"").toLowerCase().replace(/"/g,"&quot;")}">
                <td class="num">${o+1}</td>
...
})();`,y=new Blob([w],{type:"application/javascript"}),v=URL.createObjectURL(y),$=`<!DOCTYPE html>
<html lang="en">
<head>
<meta charset="UTF-8"/>
...
</html>`,_=new Blob([$],{type:"text/html"});window.open(URL.createObjectURL(_),"_blank"),console.log("\u2713 Opened visualisation tab")})();})();
